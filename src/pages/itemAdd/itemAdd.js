...
import {addItem, getItemDistinctCategoryByUserId, getItemDistinctPositionByUserId} from "./api";
...
const ItemAdd = () => {
  ...
  const onFinish = async (values) => {
    ...
    // return
    let response = await addItem(values)
  ...
  const getItemCategory = async (userId) => {
    ...
    let response = await getItemDistinctCategoryByUserId(params)
    if (response && response.list && response.list.length > 0) {
      let result = response.list.map(item => {
        return {value: item}
      })
  ...
  const getItemPosition = async (userId) => {
    ...
    let response = await getItemDistinctPositionByUserId(params)
    if (response && response.list && response.list.length > 0) {
      let result = response.list.map(item => {
        return {value: item}
      })
